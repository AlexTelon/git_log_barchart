...
git_command = f"git log --all --pretty='%ad' --date=format:'%a' {args_to_git}"
...
for line in lines:
    if line == "":
        continue
    day = line
    commit_days[day].append(day)

largest_count = max(len(values) for values in commit_days.values())
...
print(line_format.format('day', 'count', 'commits'))

for day in ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']:
    count = len(commit_days[day])
    if count == 0:
        continue
    print(line_format.format(day, count, char_bar(count)))

total_commits = sum(len(v) for k, v in commit_days.items())
